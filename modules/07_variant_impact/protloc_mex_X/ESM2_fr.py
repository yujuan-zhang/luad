"""
ESM2 特征提取核心模块 / ESM2 Feature Extraction Core Module
=============================================================
基于 ESM2 蛋白质语言模型，提取以下多种蛋白质序列特征表示：
Extracts multiple protein sequence feature representations from the ESM2 language model:

    - CLS token 特征（位置0）          / CLS token features (position 0)
    - EOS token 特征（序列末位）        / EOS token features (end of sequence)
    - Mean 均值池化特征                / Mean pooling features
    - Segment 分段均值特征（10段）      / Segment mean features (10 equal-length segments)
    - 磷酸化位点加权特征               / Phosphorylation site weighted features

每种特征维度均为 1280（ESM2-650M 输出维度）
All features have 1280 dimensions (ESM2-650M output dim)

主要类 / Main Classes:
    - Esm2LastHiddenFeatureExtractor: 从最后一层隐藏状态提取特征
    - Esm2LayerHiddenFeatureExtractor: 从指定层提取特征

依赖版本 / Dependencies:
    torch >= 1.12.1, tqdm >= 4.63.0, sklearn >= 1.0.2
"""
...
# 检查 tqdm 版本（进度条库）/ Check tqdm version (progress bar library)
try:
    import tqdm
...
# 检查 re 版本（正则表达式）/ Check re version (regular expressions)
try:
    import re
...
# 检查 sklearn 版本（用于余弦相似度）/ Check sklearn version (for cosine similarity)
try:
    import sklearn
...
# =============================================================================
# 单蛋白特征提取函数（逐条处理）
# Single-protein feature extraction (processes one protein at a time)
# 提取最后一层隐藏状态的 CLS、EOS 和 Mean 特征
# Extracts CLS, EOS, and Mean features from the last hidden layer
# =============================================================================
def get_last_hidden_features_single(X_input, tokenizer, model, sequence_name='sequence', device_choose='auto'):
    """
    逐条提取蛋白质序列的ESM2最后一层隐藏状态特征（CLS、EOS、mean）
    Extract ESM2 last hidden layer features (CLS, EOS, mean) one protein at a time.

    参数 / Parameters:
        X_input (pd.DataFrame): 含蛋白质序列的DataFrame / DataFrame containing protein sequences
        tokenizer: ESM2 tokenizer
        model: ESM2 model (with output_hidden_states=True)
        sequence_name (str): 序列所在列名 / Column name for sequences
        device_choose (str): 设备选择 'auto'/'cuda'/'cpu' / Device: 'auto'/'cuda'/'cpu'

    返回 / Returns:
        pd.DataFrame: 每行对应一个蛋白质，包含CLS/EOS/Mean特征列（各1280维）
                      Each row is a protein with CLS/EOS/Mean feature columns (1280-dim each)
    """
    X_input = X_input.reset_index(drop=True)
    X_outcome = pd.DataFrame()

    # 自动选择计算设备 / Automatically select compute device
    ...
    model.to(device)
    with torch.no_grad():  # 推理时关闭梯度，节省内存 / Disable gradients during inference to save memory
        for index, sequence in tqdm(enumerate(X_input[sequence_name]),
                                    desc='one batch for infer time',
                                    total=len(X_input[sequence_name])):
            # 对序列进行tokenize，移动到指定设备 / Tokenize sequence and send to device
            inputs = tokenizer(sequence, return_tensors="pt").to(device)
            outputs = model(**inputs)

            # 定位EOS（序列终止符）的token索引 / Locate EOS token index in the sequence
            tokens = tokenizer.convert_ids_to_tokens(inputs['input_ids'].squeeze())
            eos_position = tokens.index(tokenizer.eos_token) if tokenizer.eos_token in tokens else len(tokens) - 1

            # 取ESM2最后一层的隐藏状态 / Get last layer hidden states
            last_hidden_state = outputs.hidden_states[-1]

            # 提取不同位置的特征 / Extract features at different positions
            last_cls_token = last_hidden_state[:, 0, :]                           # CLS token（位置0）/ Position 0
            last_eos_token = last_hidden_state[:, eos_position, :]                # EOS token（序列末）/ End position
            last_mean_token = last_hidden_state[:, 1:eos_position, :].mean(dim=1) # 均值池化（排除CLS和EOS）/ Mean pooling

            features = {}

            # 展平CLS特征并按列命名 / Flatten CLS features and name columns
            cls_features = last_cls_token.squeeze().tolist()
            for i, feature in enumerate(cls_features):
                features[f"ESM2_cls{i}"] = feature

            # 展平EOS特征并按列命名 / Flatten EOS features and name columns
            eos_features = last_eos_token.squeeze().tolist()
            for i, feature in enumerate(eos_features):
                features[f"ESM2_eos{i}"] = feature

            # 展平Mean特征并按列命名 / Flatten Mean features and name columns
            ...
            del inputs, outputs
            # 仅在CUDA设备上清理显存并同步 / Clear GPU cache and sync only when using CUDA
            if device.type == 'cuda':
...
# Legacy classes have been moved to ESM2_fr_legacy.py
# Legacy classes (Esm2LastHiddenFeatureExtractor_legacy, Esm2LayerHiddenFeatureExtractor_legacy)
# have been moved to ESM2_fr_legacy.py to keep this file clean.
# 遗留类已迁移至 ESM2_fr_legacy.py，原作者要求 v0.020 前保留，请勿删除该文件。
# To use: from protloc_mex_X.ESM2_fr_legacy import Esm2LastHiddenFeatureExtractor_legacy
